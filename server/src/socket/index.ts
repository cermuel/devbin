...
export default (io: Server) => {
	...
	io.on<"connection">("connection", async (socket: MySocket) => {

		await Sessions.findByIdAndUpdate(socket.sessionId, { connected: true });
		...
		socket.on("disconnect", async () => {
			...
			if (isDisconnected) {
				...
				Sessions.findByIdAndUpdate(socket.sessionId, { connected: false });
			}
		});
		...
		socket.on("join", async (room) => {
			...
			const project = await getSingleProject(room);
			if (!project) {
				socket.emit("error", "Project not found");
				return;
			}
			if (!project.collaborators.map(i=>i.toString()).includes(socket.userId) && project.owner.toString() !== socket.userId) {
		...
		socket.on("typing", async ({room, content, file}) => {
			...
			console.log(project);
			if (!project.files.map(i=>i._id.toString()).includes(file)) {
			...
			socket.to(room).to(socket.userId).emit("typing", {file, content, userId: socket.userId});
